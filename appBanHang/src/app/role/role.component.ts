...
export class RoleComponent implements OnInit {
  ...
  pageLast:number=0;
  ItemDelete:string='';
  constructor(private toastr: ToastrService,private formBuilder: FormBuilder,private spinner: NgxSpinnerService,private roleService:RoleService) { }
  ...
  Search(){
    ...
    this.spinner.show();
    this.roleService.Search(this.Request).then((rs:any)=>{
      if(rs.status==0){
        this.Data=rs.data;
        this.total=rs.total;
        this.genArray();
  ...
  onDeleteItem()
  {
    ...
    this.roleService.DeleteItem(this.ItemDelete).then((rs:any)=>{
      if(rs.status==0){
  ...
  Save(){
    this.spinner.show();
    this.roleService.AddOrUpdate(this.Item).then((rs:any)=>{
      if(rs.status==0){
        this.toastr.success(rs.message);
  ...
  Edit(id:string){
    this.roleService.GetByIdItem(id).then((rs:any)=>{
